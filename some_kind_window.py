...
class ChangeDataHotels(QMainWindow):
    def __init__(self):
        ...
        self.input_name_hotel.move(200, 95)
        ...
        self.input_reg_hotel.move(200, 130)

        self.vector = QLabel("Направление:", self)
        self.vector.move(50, 170)
        self.vector.resize(200, 20)

        self.vectors = QComboBox(self)
        mr = MyRepository()
        s = ['популярное', 'среднее по популярности', 'непопулярное']
        for a in s:
            self.vectors.addItem(str(a))
        self.vectors.move(200, 170)

        self.startd = QLabel("Начало мертвого сезона:", self)
        self.startd.move(50, 210)
        self.startd.resize(200, 20)

        self.startds = QLineEdit(self)
        self.startds.move(200, 210)
        self.startds.resize(200, 20)
        self.startds.setInputMask("9999-99-99")

        self.endd = QLabel("Конец мертвого сезона:", self)
        self.endd.move(50, 250)
        self.endd.resize(200, 20)

        self.endds = QLineEdit(self)
        self.endds.move(200, 250)
        self.endds.resize(200, 20)
        self.endds.setInputMask("9999-99-99")
    ...
    def fill(self):
        ...
        if flag:
            ...
            self.vectors.setCurrentText(x.направление)
            self.startds.setText(str(x.начало_мертвого_сезона))
            self.endds.setText(str(x.конец_мертвого_сезона))
        else:
    ...
    def submit(self):
        ...
        if flag:
            ...
            r.название=name
            r.направление = str(self.vectors.currentText())
            a = self.startds.text()
            a = date(int(a[:a.find('-')]), int(a[a.find('-') + 1: a.rfind('-')]), int(a[a.rfind('-') + 1:]))
            r.начало_мертвого_сезона = a
            a = self.endds.text()
            a = date(int(a[:a.find('-')]), int(a[a.find('-') + 1: a.rfind('-')]), int(a[a.rfind('-') + 1:]))
            r.конец_мертвого_сезона = a
            r.регион = reg
            repo.add_something(r)
        else:
            r = Отели()
            r.id = int(id)
            r.направление = self.vectors.currentText()
            a = self.startds.text()
            a = date(int(a[:a.find('-')]), int(a[a.find('-') + 1: a.rfind('-')]), int(a[a.rfind('-') + 1:]))
            r.начало_мертвого_сезона = a
            a = self.endds.text()
    ...
    def __init__(self, l: list):
        ...
        for i in r:
            if str(i.отель) == str(l[2]) and str(i.категория) == m:
                cc = int(i.цена)
                re = repo.get_something(Отели)
                for j in re:
                    if str(j.id) == str(l[2]):
                        hhru = j.направление
                        ds = j.начало_мертвого_сезона
                        de = j.конец_мертвого_сезона
        ...
        if ds <= d1 <= de:
            prize *= 0.8
            prize = str(prize)
            prize = prize[:prize.find('.') + 3]
        elif hhru == 'непопулярное':
            prize *= 0.9
            prize = str(prize)
            prize = prize[:prize.find('.') + 3]
        self.prices.setText(str(prize))
    ...
    def submit(self):
        ...
        if d1 >= d2:
            ...
            x = Журнал_бронирования(отель=self.hotels.text(), заказчик=self.zakazs.text(), id=int(m), номер=self.rooms.text(),
                                                дата_заезда=d1, дата_выезда=d2, дата = d3, цена=self.prices.text())
        ...
        if 'новый' in x:
            ...
            if p != -1:
                repo.add_something(Цены(id = x[:x.find(' ')], отель=self.hotels.currentText(), категория=self.categs.currentText(),
                                        цена= jj[:jj.find('.') + 3]))
                m = 1
                repo.add_something(Изменения_цен(id=m, дата=datetime.today(), отель=self.hotels.currentText(),
                                                 категория=self.categs.currentText(), цена=jj[:jj.find('.') + 3]))
            else:
                repo.add_something(Цены(id = x[:x.find(' ')], отель=self.hotels.currentText(), категория=self.categs.currentText(),
                                        цена= jj))
                m = 1
                repo.add_something(Изменения_цен(id=m, дата=datetime.today(), отель=self.hotels.currentText(),
                                                 категория=self.categs.currentText(), цена=jj))
        else:
            r = repo.get_something(Цены)
            for i in r:
                if str(i.id) == str(x):
        ...
        for i in r:
            if str(i.отель) == self.ids.currentText():
                if i.категория == "стандарт":
                    ...
                    data[2] = str(i.цена)
        self.des.setText(str(data[0]))
        self.lus.setText(str(data[1]))
        self.aps.setText(str(data[2]))


class OtchetJopa(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        self.dates = QLineEdit(self)
        self.dates.move(150*2, 50)
        self.dates.resize(200, 20)
        self.dates.setInputMask('9999-99-99')
        self.dates.setText(str(datetime.today())[:10])
        ...
        for a in s:
            self.hotels.addItem(str(a.id))
        self.hotels.move(150*2, 90)
        self.hotels.currentTextChanged.connect(self.fill_all)

        self.night = QLabel("Количество проданных ночей", self)
        self.night.move(50, 130)
        self.night.resize(200, 20)

        self.nights = QLabel(self)
        self.nights.move(150*2, 130)
        self.nights.resize(200, 20)

        self.summ = QLabel("Общая сумма продаж", self)
        self.summ.move(50, 170)
        self.summ.resize(200, 20)

        self.summs = QLabel(self)
        self.summs.move(150*2, 170)
        self.summs.resize(200, 20)

        self.percent = QLabel("% загрузки номерного фонда", self)
        self.percent.move(50, 210)
        self.percent.resize(200, 20)

        self.percents = QLabel(self)
        self.percents.move(150*2, 210)
        self.percents.resize(200, 20)

        self.adr = QLabel("Ср. цена ADR", self)
        self.adr.move(50, 250)
        self.adr.resize(200, 20)

        self.adrs = QLabel(self)
        self.adrs.move(150*2, 250)
        self.adrs.resize(200, 20)

        self.revpar = QLabel("RevPar", self)
        self.revpar.move(50, 290)
        self.revpar.resize(200, 20)

        self.revpars = QLabel(self)
        self.revpars.move(150*2, 290)
        self.revpars.resize(200, 20)
        self.fill_all

    def fill_all(self):
        repo = MyRepository()
        r = repo.get_something(Журнал_бронирования)
        tete = self.dates.text()
        tete = date(int(tete[:tete.find('-')]), int(tete[tete.find('-') + 1: tete.rfind('-')]), int(tete[tete.rfind('-') + 1:]))
        nights = 0
        summ = 0
        hotel = self.hotels.currentText()
        per1 = 0
        per2 = 0
        rr = list(repo.get_something(Номера))
        for i in rr:
            if str(i.отель) == str(hotel):
                per2 += 1
        for i in r:
            if str(i.отель) == str(hotel) and i.дата == tete:
                z = (i.дата_выезда - i.дата_заезда)
                nights += z.days
                per1 += 1
                summ += float(i.цена)
        nights = nights
        self.nights.setText(str(nights))
        summ = summ
        self.summs.setText(str(summ))
        percent = per1/per2
        self.percents.setText(str(percent))
        if not(nights == 0):
            adr = summ/nights
        else:
            adr = 0
...
class ZayavkaWork(QMainWindow):
    def __init__(self):
        ...
        self.hotel = QLabel("К УБОРКЕ", self)
        self.hotel.move(50, 50)
        self.hotel.resize(200, 20)


        self.hotel = QLabel(f'{str(datetime.today())[:10]}', self)
        self.hotel.move(200, 50)
        self.hotel.resize(200, 20)
        ...
        for a in s:
            self.hotels.addItem(str(a.id))
        self.hotels.move(200, 90)
        self.hotels.currentTextChanged.connect(self.fill_all)

        self.number = QLabel("Номер к уборке:", self)
        self.number.move(50, 130)
        self.number.resize(200, 20)

        self.numbers = QComboBox(self)
        mr = MyRepository()
        s = list(mr.get_something(Номера))
        for a in s:
            z = str(a.статус)
            z = z[z.find('(') + 1: z.rfind(')')]
            if z == 'грязно' and str(a.отель) == str(self.hotels.currentText()):
                self.numbers.addItem(str(a.номер))
        self.numbers.move(200, 130)

        self.stuff = QLabel("ФИО горничной:", self)
        self.stuff.move(50, 170)
        self.stuff.resize(200, 20)

        self.stuffs = QComboBox(self)
        mr = MyRepository()
        s = list(mr.get_something(Сотрудники))
        for a in s:
            if a.вид_работ == "горничная" or a.вид_работ == "Горничная" :
                self.stuffs.addItem(str(a.фио_сотрудника))
        self.stuffs.move(200, 170)
        self.stuffs.currentTextChanged.connect(self.fill_all)

        self.status = QLabel("Статус:", self)
        self.status.move(50, 210)
        self.status.resize(200, 20)

        self.statuss = QComboBox(self)
        mr = MyRepository()
        s = ["к выполнению", "на выполнении", "выполнена"]
        for a in s:
            self.statuss.addItem(str(a))
        self.statuss.move(200, 210)
        self.statuss.currentTextChanged.connect(self.fill_all)

        self.submit_button = QPushButton("Создать заявку на уборку", self)
        self.submit_button.move(50, 250)
        self.submit_button.clicked.connect(self.submit)
        self.submit_button.resize(200, 40)
    ...
    def submit(self):
        ...
        for i in range(1, max(r) + 3):
            if not(i in r):
                m = i
                break
        repo.add_something(Работа(id=m, дата=datetime.today(), фио_горничной=self.stuffs.currentText(),
                                  отель=self.hotels.currentText(), номер_к_уборке=self.numbers.currentText(),
                                  статус=self.statuss.currentText()))
        self.close()


class AddNewStuff(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        self.fio = QLabel("ФИО сотрудника:", self)
        self.fio.move(50, 50)
        self.fio.resize(200, 20)

        self.fios = QLineEdit(self)
        self.fios.move(150, 50)
        self.fios.resize(200, 20)

        self.work = QLabel("Вид работ:", self)
        self.work.move(50, 90)
        self.work.resize(200, 20)

        self.works = QComboBox(self)
        mr = MyRepository()
        s = list(mr.get_something(Виды_работ))
        for a in s:
            self.works.addItem(str(a.наименование))
        self.works.move(150, 90)
    ...
    def submit(self):
        repo = MyRepository()
        r = repo.get_something(Сотрудники)
        r = [a.id for a in r]
        r = [0] + r
        for i in range(1, max(r) + 3):
            if not(i in r):
                m = i
                break
        repo.add_something(Сотрудники(id=m, фио_сотрудника=self.fios.text(), вид_работ=self.works.currentText()))
        self.close()


class AddNewWork(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        self.work = QLabel("Тип работы:", self)
        self.work.move(50, 50)
        self.work.resize(200, 20)

        self.works = QLineEdit(self)
        self.works.move(200, 50)
        self.works.resize(200, 20)
    ...
    def submit(self):
        repo = MyRepository()
        repo.add_something(Виды_работ(наименование=self.works.text()))
        self.close()


class OtchetUborki(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        data = list(self.repo.get_something(Работа))
        self.data = []
        t = []
        r = [[a.фио_горничной, a.статус] for a in data]
        dd = {a[0]:[0, 0, 0] for a in r}
        for i in r:
            x = i[1]
            #"к выполнению", "на выполнении", "выполнена"
            if x == "к выполнению":
                dd[i[0]][0] += 1
            elif x == "на выполнении":
                dd[i[0]][1] += 1
            else:
                dd[i[0]][2] += 1
        for i in dd:
            t = [i, dd[i][0], dd[i][1], dd[i][2]]
            self.data.append(t)

        model = MyTableModel(self.data, ["ФИО горничной", "К уборке", "На уборке", "Убрано"])
        self.table_view.setModel(model)
        self.table_view.setFixedSize(450, 450)
...
class MyTableModel(QAbstractTableModel):
    ...
    def data(self, index, role):
        if role == Qt.ItemDataRole.DisplayRole:
            if type(self._data[index.row()]) == type(list()):
                x = str(self._data[index.row()][index.column()])
                return x
            else:
                x = str(self._data[index.row()])
                return x
        return None
...
a = [ChangeDataHotels, ChangeDataRegions, ChangeDataRooms, ChangeReservRooms, CheckOtchet, CreateReserv, CreateNewGuest,
      CreateNewClient, CheckIn, CreateNewGroup, CheckOut, ChangePrices, ChartPrices, OtchetJopa, GuestToGroup, ZayavkaWork,
      AddNewStuff, AddNewWork, OtchetUborki]
